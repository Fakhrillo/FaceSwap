...
from root_folder.face_analyser import get_one_face
from root_folder.capturer import get_video_frame, get_video_frame_total
from root_folder.face_reference import get_face_reference, set_face_reference, clear_face_reference
from root_folder.predictor import predict_frame, clear_predictor
from root_folder.processors.frame.core import get_frame_processors_modules
from root_folder.utilities import is_image, is_video, resolve_relative_path
...
def select_source_path(source_path: Optional[str] = None) -> None:
    ...
    if is_image(source_path):
        root_folder.globals.source_path = source_path
        RECENT_DIRECTORY_SOURCE = os.path.dirname(root_folder.globals.source_path)
        image = render_image_preview(root_folder.globals.source_path, (200, 200))
        source_label.configure(image=image)
    else:
        root_folder.globals.source_path = None
...
def select_target_path(target_path: Optional[str] = None) -> None:
    ...
    if is_image(target_path):
        root_folder.globals.target_path = target_path
        RECENT_DIRECTORY_TARGET = os.path.dirname(root_folder.globals.target_path)
        image = render_image_preview(root_folder.globals.target_path, (200, 200))
        ...
        root_folder.globals.target_path = None
...
def select_output_path(start: Callable[[], None]) -> None:
    ...
    if is_image(root_folder.globals.target_path):
    ...
    elif is_video(root_folder.globals.target_path):
        ...
        output_path = None
    if output_path:
        root_folder.globals.output_path = output_path
        RECENT_DIRECTORY_OUTPUT = os.path.dirname(root_folder.globals.output_path)
        start()
...
def toggle_preview() -> None:
    if PREVIEW.state() == 'normal':
        ...
        PREVIEW.withdraw()
        clear_predictor()
    elif root_folder.globals.source_path and root_folder.globals.target_path:
        init_preview()
        update_preview(root_folder.globals.reference_frame_number)
        PREVIEW.deiconify()
...
def init_preview() -> None:
    ...
    if is_video(root_folder.globals.target_path):
        video_frame_total = get_video_frame_total(root_folder.globals.target_path)
        ...
        preview_slider.set(root_folder.globals.reference_frame_number)


def update_preview(frame_number: int = 0) -> None:
    if root_folder.globals.source_path and root_folder.globals.target_path:
        temp_frame = get_video_frame(root_folder.globals.target_path, frame_number)
        if predict_frame(temp_frame):
            sys.exit()
        source_face = get_one_face(cv2.imread(root_folder.globals.source_path))
        if not get_face_reference():
            reference_frame = get_video_frame(root_folder.globals.target_path, root_folder.globals.reference_frame_number)
            reference_face = get_one_face(reference_frame, root_folder.globals.reference_face_position)
...
def update_face_reference(steps: int) -> None:
    ...
    root_folder.globals.reference_face_position += steps
    root_folder.globals.reference_frame_number = reference_frame_number
